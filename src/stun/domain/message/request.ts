 ...
 */
export default class StunRequest extends StunMessage {
  kMessageType: number;
   ...
   */
  setTransactionId(transactionId: Buffer) {
   ...
   */
  addAttribute(type: number, ...arguments_) {
   ...
   * @returns {boolean}
   */
  addAddress(address: string, port: number) {
   ...
   * @returns {boolean}
   */
  addAlternateServer(address: string, port: number) {
   ...
   * @returns {boolean}
   */
  addUsername(username: string | Buffer) {
   ...
   * @returns {boolean}
   */
  addError(code: number, reason: string) {
   ...
   * @returns {boolean}
   */
  addUnknownAttributes(attributes_: number[]) {
    assertErrorType(this.type);
   ...
   */
  addFingerprint() {
    const attributeFingerprint = this.addAttribute(
   ...
   * @returns {boolean}
   */
  addIceControlled(tiebreaker: Buffer) {
   ...
   * @returns {boolean}
   */
  addIceControlling(tiebreaker: Buffer) {
    assertBindingRequest(this.type);
  ...
  setTransactionID = this.setTransactionId;
}
 ...
 * @returns {boolean} The result of an operation.
 */
function isValidTransactionId(transactionId: Buffer) {
  return (
 ...
 * @param {any} string
 */
function assert128string(string: any) {
 ...
 */
function assertErrorType(type: number) {
  const isErrorType =
 ...
 * @param {number} type Message type.
 */
function assertBindingRequest(type: number) {
